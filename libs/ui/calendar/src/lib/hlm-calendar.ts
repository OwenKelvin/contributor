...
import { BrnSelect, BrnSelectImports, BrnSelectValue } from '@spartan-ng/brain/select';
import { buttonVariants } from '@nyots/ui/button';
import { HlmIcon } from '@nyots/ui/icon';
import { HlmSelect, HlmSelectContent, HlmSelectOption, HlmSelectTrigger } from '@nyots/ui/select';
import { hlm } from '@nyots/ui/utils';
...
@Component({
  ...
  template: `
    <div
      ...
      [defaultFocusedDate]="$any(defaultFocusedDate())"
                ...
                <brn-select brnCalendarMonthSelect>
                  ...
                  <hlm-select-content class="max-h-80">
                    @for (month of _i18n.config().months(); track month) {
                      <hlm-option [value]="$any(month)">{{ month }}</hlm-option>
                    }
                  </hlm-select-content>
                </brn-select>
              </ng-template>
                ...
                <brn-select brnCalendarYearSelect>
                  ...
                  <hlm-select-content class="max-h-80">
                    @for (year of _i18n.config().years(); track year) {
                      <hlm-option [value]="$any(year)">{{ year }}</hlm-option>
                    }
                  </hlm-select-content>
                </brn-select>
              </ng-template>
          ...
          <tbody role="rowgroup">
            <tr *brnCalendarWeek="let week" class="mt-2 flex w-full">
              @for (date of week; track _dateAdapter.getTime(date)) {
                <td
                  brnCalendarCell
                  class="data-[selected]:data-[outside]:bg-accent data-[selected]:bg-accent relative size-8 p-0 text-center text-sm focus-within:relative focus-within:z-20 first:data-[selected]:rounded-l-md last:data-[selected]:rounded-r-md [&:has([aria-selected].day-range-end)]:rounded-r-md"
                >
                  <button
                    brnCalendarCellButton
                    [date]="date"
...
export class HlmCalendar<T> {
  ...
  protected readonly _btnClass = hlm(
    ...
    'data-[outside]:text-muted-foreground data-[outside]:aria-selected:bg-accent data-[outside]:aria-selected:text-muted-foreground data-[outside]:opacity-50 data-[outside]:aria-selected:opacity-30',
